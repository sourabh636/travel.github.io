...
if (isset($_POST['email']) && isset($_POST['gamename'])) {
	...
	if (empty($email)) {
		header("Location: request1.php?error=Email is required&$user_data");
	    exit();
	}
	else if(empty($gamename)){
        header("Location: request1.php?error=Name of the game is required&$user_data");
	    exit();
	}
	...
	if (mysqli_num_rows($email) > 0) {
			header("Location: request1.php?error=The email is already in use&$user_data");
	        exit();
		}
		else {
           ...
           if ($result2) {
           	 header("Location: request1.php?success=Your account has been created successfully");
	         exit();
           }
           else {
	           	header("Location: request1.php?error=unknown error occurred&$user_data");
		        exit();
           }
       }
   }
